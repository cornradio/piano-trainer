...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('DiffBox').addEventListener('click', e => {
    ...
    const songUI = document.getElementById('SongSelector');
    if (customUI) customUI.style.display = (currentDifficulty === 'custom') ? 'block' : 'none';
    if (songUI) songUI.style.display = (currentDifficulty === 'song') ? 'block' : 'none';
  ...
  if (songSelect && typeof SONGS !== 'undefined') {
    SONGS.forEach((song, i) => {
      songSelect.add(new Option(song.name, i));
    });
    songSelect.addEventListener('change', () => {
      currentSong = SONGS[parseInt(songSelect.value)];
      songIndex = 0;
      if (currentDifficulty === 'song') {
        setGameMode('rh', 'song');
      }
    });
    currentSong = SONGS[0];
